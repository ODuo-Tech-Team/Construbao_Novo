<?php
/**
 * ADMIN - Editar Categoria do Blog
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!validateCsrfToken($_POST['_token'] ?? '')) {
        ...
        if (empty($data['slug'])) {
            ...
            $exists = fetchOne('categorias_blog', 'slug = ? AND id != ?', [$data['slug'], $id]);
            if ($exists) {
        ...
        if (empty($errors)) {
            $success = update('categorias_blog', $data, $id);
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <div style="display: flex; align-items: center; gap: var(--spacing-2);">
                        ...
                        <input type="text" id="cor-text" value="<?= e($data['cor']) ?>" class="form-input" style="width: 100px;" readonly>
                    </div>
                </div>
...
<div style="margin-top: var(--spacing-6); padding: var(--spacing-4); background: var(--color-gray-100); border-radius: var(--radius-md);">
    <p style="margin: 0;">
        <strong><?= $totalPosts ?></strong> post(s) nesta categoria
    </p>
