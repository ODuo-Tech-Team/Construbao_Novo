...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!validateCsrfToken($_POST['_token'] ?? '')) {
        ...
        if (empty($data['slug'])) {
            ...
            $exists = fetchOne('equipamentos', 'slug = ? AND id != ?', [$data['slug'], $id]);
            if ($exists) {
        ...
        if (!empty($_FILES['imagem']['name'])) {
            $novaImagem = uploadImage($_FILES['imagem'], 'equipamentos');
            if ($novaImagem === null) {
        ...
        if (empty($errors)) {
            ...
            if ($novaImagem && $equipamento['imagem']) {
            ...
            $success = update('equipamentos', $data, $id);

            if ($success) {
                setFlash('success', 'Equipamento atualizado com sucesso!');
                redirect(SITE_URL . '/admin/equipamentos/');
            } else {
                $errors[] = 'Erro ao atualizar equipamento. Tente novamente.';
                // Manter texto de características para o formulário
...
<div class="admin-page-header">
    <h1 class="admin-page-title">Editar Equipamento</h1>
    <div style="display: flex; gap: var(--spacing-2);">
        <a href="<?= SITE_URL ?>/equipamento.php?slug=<?= $equipamento['slug'] ?>" target="_blank" class="btn btn-outline">
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <div class="form-section">
            ...
            <div class="form-group">
                ...
                <div class="image-upload <?= $equipamento['imagem'] ? 'has-image' : '' ?>">
                    <input type="file" name="imagem" accept="image/*" style="display: none;">
                    <?php if ($equipamento['imagem']): ?>
                    <img src="<?= imageUrl($equipamento['imagem'], 'equipamentos') ?>" alt="<?= e($equipamento['nome']) ?>" class="image-preview">
                    <?php endif; ?>
                    <div class="image-upload-placeholder" <?= $equipamento['imagem'] ? 'style="display: none;"' : '' ?>>
...
<div style="margin-top: var(--spacing-6); padding: var(--spacing-4); background: var(--color-gray-100); border-radius: var(--radius-md);">
    <small style="color: var(--color-text-muted);">
        Criado em: <?= formatDate($equipamento['created_at'], 'd/m/Y H:i') ?> |
        Atualizado em: <?= formatDate($equipamento['updated_at'], 'd/m/Y H:i') ?>
    </small>
</div>
