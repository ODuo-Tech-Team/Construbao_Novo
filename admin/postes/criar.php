...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!validateCsrfToken($_POST['_token'] ?? '')) {
        ...
        if (empty($data['slug'])) {
            ...
            $exists = fetchOne('postes', 'slug = ?', [$data['slug']]);
            if ($exists) {
        ...
        if (!empty($_FILES['imagem']['name'])) {
            $imagem = uploadImage($_FILES['imagem'], 'postes');
            if ($imagem === null) {
        ...
        if (empty($errors)) {
            $data['imagem'] = $imagem;
            $id = insert('postes', $data);

            if ($id) {
                setFlash('success', 'Poste criado com sucesso!');
                redirect(SITE_URL . '/admin/postes/');
            } else {
                $errors[] = 'Erro ao criar poste. Tente novamente.';
                if ($imagem) {
                    deleteImage($imagem, 'postes');
                }
            }
        }
    }
...
<div class="admin-page-header">
    <h1 class="admin-page-title">Novo Poste</h1>
